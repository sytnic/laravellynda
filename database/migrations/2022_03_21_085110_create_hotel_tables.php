...
class CreateHotelTables extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('rooms', function (Blueprint $table) {
            ...
            $table->foreign('room_type_id')->references('id')->on('room_types');  // внешний ключ, столбец -> таблица
        });

        Schema::create('discounts', function (Blueprint $table) {
            $table->bigIncrements('id')->comment('The Primary Key for the table.');
            $table->string('name', 255);
            $table->string('code', 50)->comment('The code someone would be expected to enter at checkout.'); // промокод
            $table->unsignedInteger('discount')->comment('The discount in whole cents for a room.');  // скидка в центах
            $table->timestamps();
            $table->softDeletes();
        });
        ...
        Schema::create('rates', function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('room_type_id')->index('room_type_id')->comment('The corresponding room type.');
            $table->boolean('is_weekend')->default(false)->comment('If this is the weekend rate or not.'); // булево, по умолчанию false
            $table->timestamps();

            $table->unique(['room_type_id', 'is_weekend']); // задано сочетание двух столбцов в этой таблице как уникальное
